  ...
  return (
    <div className={cn('w-full', disabled && 'opacity-50 pointer-events-none')}>
      ...
      <button
        ...
        disabled={disabled}
        className={cn(
          'flex w-full items-center gap-3 px-4 py-2.5 text-[0.875rem] transition-all group cursor-pointer text-left',
          'hover:bg-muted/50',
      ...
      >
        <span
          className={cn(
            'shrink-0 [&>svg]:w-5 [&>svg]:h-5 [&>svg]:stroke-[1.5] transition-opacity group-hover:opacity-80',
            variant === 'default' ? 'text-icon-menu' : 'text-destructive'
          )}
        >
          {icon}
        </span>
        <div className='flex-1 min-w-0 flex flex-col'>
          <div className='flex items-center gap-1.5'>
            <span className={cn('truncate', variant === 'default' ? 'text-foreground' : 'text-destructive')}>
              {label}
            </span>
            {labelRightElement}
          </div>
