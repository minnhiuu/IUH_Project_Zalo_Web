...
export const ImageUpload = ({
  ...
  const handleFile = (file: File) => {
    if (file && file.type.startsWith('image/')) {
      ...
      toast.error(texts.error.invalidType)
    }
  }
        ...
        {preview ? (
          ...
          <div className='flex flex-col items-center justify-center pointer-events-none p-4 text-center'>
            ...
            <div className='flex flex-col gap-1'>
              <span className='text-sm font-medium text-foreground'>{texts.empty.title}</span>
              <span className='text-[11px] text-muted-foreground'>{texts.empty.hint}</span>
            </div>
          </div>
        )}
