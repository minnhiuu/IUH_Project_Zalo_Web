...
export function StoryCard({
  ...
  return (
    <button
      type='button'
      onClick={onClick}
      className='group relative h-[250px] w-[140px] shrink-0 overflow-hidden rounded-[12px] bg-zinc-100 text-left shadow-sm ring-1 ring-zinc-200/50 transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:bg-zinc-900 dark:ring-white/10'
    >
      {mediaUrl ? (
        mediaType === 'VIDEO' ? (
          <StoryVideoPlayer
      ...
      <div className='absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-black/80 to-transparent p-3 pt-10'>
        <p className='line-clamp-2 text-[13px] font-semibold text-white drop-shadow-md'>{authorName}</p>
      </div>
    </button>
  )
}
