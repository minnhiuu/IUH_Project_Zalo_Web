...
export function LogoutConfirmDialog({ open, onOpenChange }: LogoutConfirmDialogProps) {
  ...
  const handleLogout = async () => {
    ...
    console.info('[LogoutDebug] Logout confirm accepted', {
    ...
    if (fcmToken) {
      void unregisterDeviceMutation
        .mutateAsync(fcmToken)
        .then(() => {
          console.info('[LogoutDebug] FCM device unregistered')
        })
        .catch((error) => {
          console.warn('[LogoutDebug] Failed to unregister FCM device', error)
        })
    }
    ...
    void logoutMutation
      .mutateAsync({})
      .then(() => {
        console.info('[LogoutDebug] Remote logout API completed')
      })
      .catch((error) => {
        console.warn('[LogoutDebug] Remote logout API failed', error)
      })
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <DialogFooter className='flex flex-row justify-end gap-3 px-4 pb-4'>
          ...
          <Button
            onPointerDown={() => {
              console.info('[LogoutDebug] Confirm button pointer down')
            }}
