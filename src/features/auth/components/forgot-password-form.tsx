        ...
        {isRequestStep ? (
          ...
          <form onSubmit={resetForm.handleSubmit(onHandleReset)} className='w-full space-y-6 px-5'>
            ...
            <div className='flex items-center justify-between py-2'>
              <span className='text-[14px] font-medium text-foreground cursor-pointer'>
                Log out other running sessions
              </span>
              <Controller
                ...
                render={({ field }) => (
                  <button
                    ...
                    className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${field.value ? 'bg-primary' : 'bg-muted'
                      }`}
                  >
                    <span
                      aria-hidden='true'
                      className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-background shadow ring-0 transition duration-200 ease-in-out ${field.value ? 'translate-x-4' : 'translate-x-0'
                        }`}
                    />
                  </button>
                )}
