...
export function AppearanceSettings() {
  ...
  const { pending, updateAppearanceSettings } = useSettingsState()
  ...
  const handleThemeChange = (newTheme: 'light' | 'dark' | 'system') => {
    setTheme(newTheme)
    if (newTheme !== 'system') {
      updateAppearanceSettings({ theme: newTheme === 'light' })
    }
  }
  ...
  return (
    <div className='space-y-4'>
      ...
      <ActionRow title={text.settings.appearance.theme.title} description={text.settings.appearance.theme.description}>
        <div className='grid grid-cols-3 gap-3 mt-3'>
          ...
          <button
            ...
            className={cn(
              'relative flex flex-col items-center gap-3 p-4 rounded-lg border-2 transition-all hover:border-primary/50 disabled:opacity-60 disabled:cursor-not-allowed',
