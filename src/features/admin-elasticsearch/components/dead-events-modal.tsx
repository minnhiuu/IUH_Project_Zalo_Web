...
import { Calendar, Info, Terminal, AlertCircle } from 'lucide-react'
import type { DeadEvent } from '../schemas/elasticsearch.schema'
import { useElasticsearchText } from '../i18n/use-elasticsearch-text'
import { formatFullDateTime } from '@/utils/date'

interface DeadEventDetailModalProps {
  isOpen: boolean
  onOpenChange: (open: boolean) => void
  eventId: string | null
  events: DeadEvent[] | undefined
}

export function DeadEventDetailModal({ isOpen, onOpenChange, eventId, events }: DeadEventDetailModalProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-[700px] p-0 overflow-hidden rounded-xl border-none shadow-2xl bg-white dark:bg-slate-900'>
        <DialogHeader className='p-8 pb-4 bg-gray-50/50 dark:bg-slate-800/30'>
          ...
          <DialogDescription className='text-sm font-medium mt-2 text-gray-500 dark:text-slate-400'>
          ...
          <div className='p-8 pt-4 space-y-6'>
              ...
              <div className='space-y-4'>
                <div className='flex items-start gap-4'>
                  ...
                  <div className='space-y-1'>
                    <p className='text-sm text-gray-500 font-medium'>{text.dlq.table.aggregateId}</p>
            ...
            <div className='pt-2'>
              <Button
                ...
                className='w-full h-12 rounded-xl border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800'
              >
