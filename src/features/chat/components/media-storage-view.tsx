...
function groupByDate(messages: MessageResponse[], dateLabelFn: (day: number, month: number) => string): { dateLabel: string; items: MessageResponse[] }[] {
...
export function MediaStorageView({ conversationId, members, defaultTab = 'media', onClose }: MediaStorageViewProps) {
  ...
  const groupedFiles = useMemo(() => groupByDate(filteredFiles, text.mediaStorage.dateLabel), [filteredFiles, text.mediaStorage.dateLabel])
  ...
  return (
    <div className='flex flex-col h-full bg-background'>
      ...
      <div className='h-[68px] flex items-center border-b border-border shrink-0 px-4 relative'>
        ...
        <h2 className='absolute left-1/2 -translate-x-1/2 font-bold text-[16px] text-foreground'>{text.mediaStorage.title}</h2>
      </div>
...
function SenderFilter({ members, value, onChange, label }: SenderFilterProps) {
  ...
  return (
    <div ref={ref} className='relative'>
      ...
      >
        {selected && (
          <UserAvatar src={selected.avatar ?? undefined} name={selected.fullName} className='w-4 h-4 text-[8px] shrink-0' />
        )}
        <span className='max-w-[80px] truncate'>{selected?.fullName ?? label}</span>
        {value ? (
          <X size={12} onClick={(e) => { e.stopPropagation(); onChange(null) }} className='cursor-pointer' />
        ) : (
          <ChevronDown size={13} />
        )}
      </button>
        ...
        <div className='absolute top-full left-0 mt-1 w-48 bg-background border rounded-xl shadow-xl z-20 py-1 max-h-48 overflow-y-auto'>
          {members.map(m => (
            <button
              key={m.userId}
              type='button'
              onClick={() => { onChange(value === m.userId ? null : m.userId); setOpen(false) }}
              className={cn(
...
function DateFilter({ value, onChange, label }: DateFilterProps) {
  ...
  const clear = () => { onChange(null); setFrom(''); setTo(''); setOpen(false) }
  ...
  return (
    <div ref={ref} className='relative'>
      ...
      >
        ...
        {isActive ? (
          <X size={12} onClick={(e) => { e.stopPropagation(); clear() }} className='cursor-pointer' />
        ) : (
          <ChevronDown size={13} />
        )}
      </button>
      {open && (
        <div className='absolute top-full left-0 mt-1 w-56 bg-background border rounded-xl shadow-xl z-20 p-3 flex flex-col gap-2'>
          ...
          <div className='flex gap-2 pt-1'>
            <button type='button' onClick={clear} className='flex-1 text-[13px] py-1.5 rounded-lg border hover:bg-muted transition-colors'>Xoá</button>
            <button type='button' onClick={apply} className='flex-1 text-[13px] py-1.5 rounded-lg bg-primary text-white hover:bg-primary/90 transition-colors'>Áp dụng</button>
          </div>
        </div>
      )}
    </div>
