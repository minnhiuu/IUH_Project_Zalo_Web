...
export function resolvePromoteAdminAction(context: ActionContext): ActionResolveResult {
  ...
  if (isTargetMe) {
    return { i18nKey: 'chat.system.promote_admin.self_promoted' }
  }

  if (isActorMe) {
    return {
      i18nKey: 'chat.system.promote_admin.by_you',
  ...
  return {
    i18nKey: 'chat.system.promote_admin.by_actor',
    values: { target: targetName },
    clickableTargetIds
  }
}
