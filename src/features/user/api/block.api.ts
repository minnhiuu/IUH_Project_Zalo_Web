...
export const blockApi = {
  blockUser: (body: BlockUserRequest) => 
    http.post<ApiResponse<BlockedUserResponse>>('/blocks', body),

  unblockUser: (blockedUserId: string) => 
    http.delete<ApiResponse<void>>(`/blocks/${blockedUserId}`),
  ...
  getMyBlockedUsers: () => 
    http.get<ApiResponse<BlockedUserResponse[]>>('/blocks'),

  getMyBlockedUsersWithDetails: () => 
    http.get<ApiResponse<BlockedUserDetailResponse[]>>('/blocks/details'),

  isUserBlocked: (userId: string) => 
    http.get<ApiResponse<boolean>>(`/blocks/${userId}/check`),

  getBlockDetails: (blockedUserId: string) => 
    http.get<ApiResponse<BlockedUserResponse>>(`/blocks/${blockedUserId}`)
}
